...
def wrap_admin_wrapper_payload(payload: bytes) -> bytes:
    """
    Minimal admin.Wrapper-compatible envelope:
      field #1 (length-delimited bytes) carries the raw message payload.
    """
...
class V2VBeaconGenerator:
    def __init__(self, cfg: V2VFrameConfig) -> None:
        ...
        self._next_besteffort_seqnum = cfg.besteffort_seqnum
    ...
    def build_frame(self):
        ...
        if self._next_besteffort_seqnum is not None:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=0,
        help="Session id integer, or 'none'/'null' to omit session_id field",
    )
